...
const links: { to: string; label: string; icon: IconName }[] = [
 { to: '/dashboard', label: 'Dashboard', icon: 'home' }, { to: '/clientes', label: 'Clientes', icon: 'users' }, { to: '/auditores', label: 'Auditores', icon: 'users' },
...
export function AppLayout() {
 ...
 const location = useLocation(); const isClients = /^\/(clientes|clients)(\/|$)/.test(location.pathname); const isAuditors = /^\/auditores(\/|$)/.test(location.pathname);
 return <div className={`dashboard-shell${isClients || isAuditors ? " clients-shell" : ""}`} onKeyDown={event => { if (event.key === 'Escape') setOpen(false); }}><Sidebar open={open} close={() => setOpen(false)} /><div className="dashboard-workspace"><DashboardHeader open={open} toggle={() => setOpen(!open)} /><main className="dashboard-main"><Outlet /></main></div></div>;
}
