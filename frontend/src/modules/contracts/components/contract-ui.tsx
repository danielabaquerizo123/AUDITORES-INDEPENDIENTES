...
export function GenerateMenu({id,label=false}:{id:string;label?:boolean}){
 const [open,setOpen]=useState(false),[busy,setBusy]=useState<''|'docx'|'pdf'>(''),[error,setError]=useState<unknown>();const ref=useRef<HTMLDivElement>(null);
 ...
 async function generate(format:'docx'|'pdf'){setOpen(false);setBusy(format);setError(undefined);try{const doc=await contractsApi.generateDocument(id,format);await contractsApi.downloadDocument(id,doc)}catch(e){setError(e)}finally{setBusy('')}}
 const status=busy==='docx'?'Generando Word...':busy==='pdf'?'Generando PDF...':'';
 return <div className="contract-download" ref={ref} onKeyDown={e=>{if(e.key==='Escape')setOpen(false)}}><button className="contract-button contract-primary" aria-label="Generar o descargar contrato" aria-expanded={open} aria-haspopup="menu" disabled={!!busy} onClick={()=>setOpen(!open)}><DownloadIcon/>{label&&(status||'Generar / Descargar')}</button>{open&&<div className="contract-menu" role="menu"><button role="menuitem" disabled={!!busy} onClick={()=>void generate('docx')}><span aria-hidden="true" className="contract-word">W</span>Generar Word</button><button role="menuitem" disabled={!!busy} onClick={()=>void generate('pdf')}><span aria-hidden="true" className="contract-pdf">PDF</span>Generar PDF</button></div>}{status&&<span role="status" className="contract-generation-status">{status}</span>}{!!error&&<ContractError error={error}/>}</div>
}
